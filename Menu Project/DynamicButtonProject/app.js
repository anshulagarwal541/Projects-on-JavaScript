...
let itemLists = document.querySelector('.itemLists')
let menuButtons = document.querySelector('.menuButtons')

window.addEventListener('DOMContentLoaded', function () {
    let category = menus.reduce(function (values, item) {
        if (!values.includes(item.category)) {
            values.push(item.category)
        }
        return values
    }, ["all"])
    menuButtons.innerHTML = getButtons(category)
    let catalog = getCatalog(menus)
    itemLists.innerHTML = catalog
    ...
    buttons.forEach(function (button) {
        button.addEventListener('click', function (e) {
            ...
            if (id == 'all') {
                ...
                let filterCatalog = menus.filter(function (item) {
                    if (item.category == id) {
                        return item
                    }
                })
                filterCatalog = getCatalog(filterCatalog)
                itemLists.innerHTML = filterCatalog
            }
        })
    })
})

function getButtons(itemList) {
    let i = itemList.map(function (item) {
        return `
        <button class="button" data-id="${item}">${item}</button>
        `
    })
    i = i.join("")
    return i
...
function getCatalog(menuList) {
    let i = menuList.map(function (item) {
